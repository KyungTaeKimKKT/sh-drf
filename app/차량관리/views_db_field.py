...
### const
TABLE_HEADER_차량관리_기준정보 = ['id', '법인명','차종','차량번호','is_exist','공급업체','약정운행거리','차량가격',
		'보증금', '타이어규격', '대여료_VAT포함','초과거리부담금','시작일','종료일', 'view_users_m2m','write_users_m2m','admin_users_m2m']  ## 😀삭제 : 'is_master적용','is_개인별할당',

COLS_WIDTH_차량관리_기준정보 = {
                        # '구분'  : 16*10,
                        # '항목' : 16*10,
                        # '정의'  : 16*100,
                    }

TABLE_HEADER_차량관리_운행DB = ['id', '차량번호_fk', '차량번호','법인명', '차종','공급업체','시작일','종료일','약정운행거리','일자','주행거리','정비금액',
		'정비사항', '관련근거', ]  ## 😀삭제 : 'is_master적용','is_개인별할당',

COLS_WIDTH_차량관리_운행DB = {
                        # '구분'  : 16*10,
                        # '항목' : 16*10,
                        # '정의'  : 16*100,
                    }
...
class DB_Field_차량관리_기준정보_View(APIView):
	...
	_serializer = serializers.차량관리_기준정보_Serializer()
	serializer_field = Util.get_Serializer_field_type(_serializer)

	차량번호list =  set(list(models.차량관리_기준정보.objects.values_list('차량번호', flat=True)))
	res_dict = {
        ###################################
		'차량번호list' :  차량번호list, 
		...
		'table_config' : {
            #############
			'table_header' : TABLE_HEADER_차량관리_기준정보 ,
			'no_Edit_cols' :['id', ],
			...
			'hidden_columns' :[],
			'cols_width' : COLS_WIDTH_차량관리_기준정보,
			'no_vContextMenuCols' : TABLE_HEADER_차량관리_기준정보 , ### vContext menu를 생성하지 않는 col.
            'no_hContextMenuRows' :[],
			'v_Menus' : {
			},
			'h_Menus' : {
			...
			'cell_Menus':{
				...
				'사용자관리': {
					"position": (
						['write_users_m2m'],	### table_header name
				...
				'관리자관리': {
					"position": (
						['admin_users_m2m'],	### table_header name
						['all']		### 전체 경우, 'all' 아니면 rowNo
					),
					"icon" : "QtGui.QIcon(':/table-icons/tb-insert_row')",
					"title": "관리자관리",
					"tooltip" :"관리(운행정보를 등록가능)자를 등록",
					"objectName" : '관리자관리',
					"enabled" : True,
				},
			}
...
class DB_Field_차량관리_운행DB_관리자_View(APIView):
	""" DB FILED VIEW {NAME:TYPE}"""
	...
	_serializer = serializers.차량관리_운행DB_Serializer()
	serializer_field = Util.get_Serializer_field_type(_serializer)
	...
	res_dict = {
		...
		'table_config' : {
            #############
			'table_header' : TABLE_HEADER_차량관리_운행DB ,
			'no_Edit_cols' :['id', ],
			'no_Edit_rows' : [], ### row index : 0,1,2 들어감
			'no_Edit_cols_color' : "QtCore.QVariant( QtGui.QColor( QtCore.Qt.GlobalColor.darkGray ))",
			# 'no_Edit_rows_color' : "QtCore.QVariant( QtGui.QColor( QtCore.Qt.GlobalColor.darkGray ))",
			...
			'hidden_columns' :[],
			'cols_width' : COLS_WIDTH_차량관리_운행DB,
			'no_vContextMenuCols' : TABLE_HEADER_차량관리_운행DB , ### vContext menu를 생성하지 않는 col.
            'no_hContextMenuRows' :[],
			'v_Menus' : {
			},
			'h_Menus' : {
...
class DB_Field_차량관리_운행DB_사용자_View(APIView):
	""" DB FILED VIEW {NAME:TYPE}"""
	...
	res_dict = {
		...
		'table_config' : {
            #############
			'table_header' : TABLE_HEADER_차량관리_운행DB ,
			'no_Edit_cols' :['id', '법인명', '차종','공급업체','시작일','종료일','약정운행거리','일자', ],
			'no_Edit_rows' : [], ### row index : 0,1,2 들어감
			'no_Edit_cols_color' : "QtCore.QVariant( QtGui.QColor( QtCore.Qt.GlobalColor.darkGray ))",
			# 'no_Edit_rows_color' : "QtCore.QVariant( QtGui.QColor( QtCore.Qt.GlobalColor.darkGray ))",
			'no_Edit_rows_color' : "QtCore.QVariant( QtGui.QColor( '#D4D4D4' ))",
			
			'hidden_columns' :[ '차량번호_fk' , ],
			'cols_width' : COLS_WIDTH_차량관리_운행DB,
			'no_vContextMenuCols' : TABLE_HEADER_차량관리_운행DB , ### vContext menu를 생성하지 않는 col.
            'no_hContextMenuRows' :[],
			'v_Menus' : {
