...
class CustomTokenObtainPairSerializer(TokenObtainPairSerializer):
    ...
    def validate(self, attrs):
        # user가 이미 존재하면 authenticate 생략
        user = getattr(self, 'user', None) or self.context.get('user')
        self.user = user
        if user is None:
            # 기존 flow: username/password authenticate
        ...
        elif self.user.user_mailid == 'rtsp_cam_service_bot':
            # 100년 후 날짜 계산
        ...
        access.payload['exp'] = int(end_of_day.timestamp())
        ...
        data['ws_url_db'] = CacheManager.get_ws_url_db_all()
        data['is_app_admin'] = self.get_is_app_admin()
        data['is_table_config_admin'] = self.get_is_table_config_admin()
        print ('login : ', data)
        return data
...
class UserSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        validated_data['password'] = make_password(self.default_password)
...
class Portal_Permission_Serializer(serializers.ModelSerializer):
    class Meta:
        model = Portal_Permission
        fields = '__all__'
        read_only_fields = ['id']